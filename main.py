...
def main() -> None:
    ...
    if not input_rows:
        logger.info("No input rows found.")
        return
    ...
    for row_idx, row in enumerate(input_rows, start=2):
        ...
        logger.info("Processing row=%s ticker=%s refresh_full=%s", row_idx, ticker, refresh_full)
        ...
        try:
            ...
            logger.exception("Failed row=%s ticker=%s", row_idx, ticker)
    ...
    logger.info("Completed. rows=%s", len(input_rows))
